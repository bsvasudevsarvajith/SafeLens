...
import { User, Mail, Shield, Key, LogOut, CheckCircle, AlertCircle, Phone, Lock } from "lucide-react";
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 max-w-4xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
          <div className="space-y-1">
            <h1 className="text-2xl sm:text-3xl font-extrabold text-white">⚙️ User Profile & Account Settings</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-navy-800 border border-navy-700 rounded-3xl p-6 space-y-4 shadow-xl">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-blue-600 via-indigo-500 to-emerald-500 flex items-center justify-center font-bold text-lg text-white shadow-md">
                  {user?.name?.[0] || user?.email?.[0]?.toUpperCase() || "U"}
                </div>
                <div>
                  <h3 className="font-bold text-lg text-white">{user?.name || "Registered User"}</h3>
                  <p className="text-xs text-gray-400">{user?.email || "user@wsrs.in"}</p>
                </div>
              </div>
